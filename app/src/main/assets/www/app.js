...
function closeNP() {
    ...
    // Lyrics View 활성 중이면 먼저 해제
    if (NP_LV.active) _exitLyricsView();
    document.getElementById('np').classList.remove('on');
    _stopLyricsTick();
}
...
function _enterNpLandscape() {
    if (NP_LV.active) _exitLyricsView(); // Lyrics View와 동시 진입 방지
    NP_LS.active = true;
...
function _renderLyrics() {
    ...
    if (NP_LV.active) { _buildLvLyricsDOM(); _ensureLvTranslations(LY.videoId, LY.lines); }
}
...
function _showLyricsLoading() {
    ...
    if (NP_LV.active) {
        document.getElementById('np-lv-lyrics')?.remove();
        _showLvStatus('가사 불러오는 중...');
    }
}
...
function _showLyricsEmpty() {
    ...
    if (NP_LV.active) _showLvStatus('가사를 찾을 수 없습니다');
}
...
function _clearLyrics() {
    ...
    if (LY._lvSpring) { LY._lvSpring.stop(); LY._lvSpring = null; }
    if (NP_LS._scrollSpring) { NP_LS._scrollSpring.stop(); NP_LS._scrollSpring = null; }
    LY.lines = [];
    LY.curIdx = -1;
    LY.videoId = null;
...
function _syncLyrics() {
    ...
    if (NP_LV.active) _highlightLvLine(found);
}
...
/* ════════════════════════════════════════════
   APPLE MUSIC 스타일 LYRICS VIEW (모바일 전용 가사 전체화면)
   ────────────────────────────────────────────
   · 기존 #np.fullscreen(PC) / NP_LS(Android 랜드스케이프) 로직과는
     완전히 독립된 레이어. 별도의 id/class(np-lv-*)와 별도의 state
     객체(NP_LV)를 사용하므로 기존 기능을 전혀 건드리지 않는다.
   · 애니메이션 원리는 기존 #np-fs-lyrics 슬롯 기반 블러/투명도/스케일
     기법을 그대로 인용하되, 모바일 단일 컬럼 화면에 맞게 폰트 크기/
     보이는 줄 수만 재조정했다.
   · 각 줄은 고정 그리드(top = i*ROW_H)가 아니라 일반 문서 흐름(static)
     으로 쌓이고, 실제 렌더된 높이(offsetTop/offsetHeight)를 기준으로
     스프링이 이동한다 → 긴 가사가 2줄 이상으로 자동 줄바꿈되어도
     겹치거나 어긋나지 않는다.
   · 외국어 가사에는 한 줄 아래에 작게 번역이 붙는다(Lyrics View에만
     적용). 번역은 무료 공개 번역 엔드포인트를 클라이언트에서 직접
     호출해 API 키/과금 없이 무제한으로 동작한다.
   · 손가락으로 위아래 드래그하면 기존 미니 가사 패널과 동일한
     _attachLyricDragScroll 로 수동 스크롤이 가능하고, 손을 떼면
     잠시 후 현재 재생 줄로 스프링 복귀한다.
════════════════════════════════════════════ */
const NP_LV = { active: false };

const LV_SLIDE = 26;
const LV_GAP = 26; // 줄 사이 간격 (margin-bottom, CSS와 일치)
const LV_SLOTS = [
    { d: -2, op: 0.28, bl: 3.0, sc: 0.90 },
    { d: -1, op: 0.55, bl: 1.0, sc: 0.95 },
    { d: 0,  op: 1.00, bl: 0.0, sc: 1.00 },
    { d: 1,  op: 0.55, bl: 1.0, sc: 0.95 },
    { d: 2,  op: 0.28, bl: 3.0, sc: 0.90 },
];
const LV_TRANS = [
...
function toggleLyricsView() {
    if (NP_LV.active) { _exitLyricsView(); return; }
    _enterLyricsView();
}

function _enterLyricsView() {
    if (NP_LS.active) return; // 랜드스케이프 모드와 동시 진입 방지
    NP_LV.active = true;
    document.getElementById('np')?.classList.add('lyrics-view-mode');
    document.getElementById('np-lyrics-btn')?.classList.add('on');
    if (LY.lines.length > 0) {
        _buildLvLyricsDOM();
        requestAnimationFrame(() => { _highlightLvLine(LY.curIdx, true); });
        if (LY.videoId) _ensureLvTranslations(LY.videoId, LY.lines);
    } else {
        _showLvStatus(LY._fetching ? '가사 불러오는 중...' : '가사를 찾을 수 없습니다');
    }
}

function _exitLyricsView() {
    NP_LV.active = false;
    document.getElementById('np')?.classList.remove('lyrics-view-mode');
    document.getElementById('np-lyrics-btn')?.classList.remove('on');
    if (LY._lvSpring) { LY._lvSpring.stop(); LY._lvSpring = null; }
    document.getElementById('np-lv-lyrics')?.remove();
}

function _showLvStatus(msg) {
    document.getElementById('np-lv-lyrics')?.remove();
    const np = document.getElementById('np');
    if (!np) return;
    const container = document.createElement('div');
    container.id = 'np-lv-lyrics';
    container.style.display = 'block';
    const status = document.createElement('div');
    status.className = 'np-lv-status';
    status.textContent = msg;
    container.appendChild(status);
    np.appendChild(container);
}

function _buildLvLyricsDOM() {
    document.getElementById('np-lv-lyrics')?.remove();
    const np = document.getElementById('np');
    if (!np) return;
    _lvLastIdx = -1;
    ...
    container.id = 'np-lv-lyrics';
    const inner = document.createElement('div');
    inner.id = 'np-lv-lyrics-inner';
    container.appendChild(inner);
    np.appendChild(container);

    LY.lines.forEach((line, i) => {
        const el = document.createElement('div');
        el.className = 'np-lv-lyric-line';
        el.style.transition = 'none';
        el.style.opacity = '0';
        el.style.filter = 'blur(8px)';
        el.style.transform = `scale(0.88) translateY(${LV_SLIDE}px)`;
        el.style.willChange = 'transform, opacity, filter';

        const orig = document.createElement('div');
        orig.className = 'np-lv-orig';
        orig.textContent = line.text;
        el.appendChild(orig);

        const trans = document.createElement('div');
        trans.className = 'np-lv-trans';
        el.appendChild(trans);

        el.addEventListener('click', e => seekToLyric(line.start + 0.05, e));
        inner.appendChild(el);
        LY.lines[i].el_lv = el;
        LY.lines[i].el_lv_trans = trans;
    });

    inner.style.transform = 'translateY(0px)';
    _attachLyricDragScroll(
        document.getElementById('np-lv-lyrics'),
        () => document.getElementById('np-lv-lyrics-inner'),
        () => LY._lvSpring,
...
let _lvLastIdx = -1;
let _lvVisible = new Set();

function _highlightLvLine(idx, instant) {
    if (!NP_LV.active || !LY.lines.length) return;
    const ai = idx;
    const dir = (_lvLastIdx < 0) ? 1 : (idx >= _lvLastIdx ? 1 : -1);
    _lvLastIdx = idx;

    const inner = document.getElementById('np-lv-lyrics-inner');
    const container = document.getElementById('np-lv-lyrics');
    if (!inner || !container) return;

    // 실제 렌더된 위치(offsetTop) 기준으로 중앙 정렬 — 줄바꿈으로 줄 높이가
    // 저마다 달라도 항상 정확히 현재 줄을 화면 중앙(패널 위쪽 가시영역)에 맞춘다.
    const activeEl = ai >= 0 ? LY.lines[ai]?.el_lv : null;
    if (activeEl) {
        const containerH = container.offsetHeight;
        const panelH = document.querySelector('.np-panel')?.offsetHeight || 0;
        const visibleH = Math.max(containerH - panelH, 200);
        const padTop = parseFloat(getComputedStyle(container).paddingTop) || 0;
        const target = Math.round((visibleH / 2) - padTop - (activeEl.offsetTop + activeEl.offsetHeight / 2));
        if (!LY._lvSpring) LY._lvSpring = _makeSpring();
        const applyFrame = v => { inner.style.transform = `translateY(${v}px)`; };
        if (instant) LY._lvSpring.jump(target, applyFrame);
        else LY._lvSpring.to(target, applyFrame);
    }
    ...
    LY.lines.forEach((line, i) => {
        const el = line.el_lv;
        if (!el) return;
        el.classList.toggle('lv-active', i === ai);
        const d = i - ai;
        const slot = LV_SLOTS.find(s => s.d === d);
        const wasVisible = _lvVisible.has(i);
        if (!slot) {
            if (wasVisible) {
                el.style.transition = LV_TRANS;
                el.style.opacity = '0';
                el.style.filter = 'blur(8px)';
                el.style.transform = `scale(0.88) translateY(${dir > 0 ? -LV_SLIDE : LV_SLIDE}px)`;
            }
            return;
        }
        newVisible.add(i);
        if (!wasVisible) {
            ...
            el.style.opacity = '0';
            el.style.filter = `blur(${Math.max(slot.bl, 3)}px)`;
            el.style.transform = `scale(${slot.sc * 0.92}) translateY(${dir > 0 ? LV_SLIDE : -LV_SLIDE}px)`;
            void el.offsetHeight;
            el.style.transition = LV_TRANS;
        } else {
            el.style.transition = LV_TRANS;
        ...
        el.style.opacity = String(slot.op);
        el.style.filter = slot.bl > 0 ? `blur(${slot.bl}px)` : 'none';
        el.style.transform = `scale(${slot.sc}) translateY(0px)`;
    });
    _lvVisible = newVisible;
}

/* ── 가사 번역 (Lyrics View 전용) ─────────────────────────────
   · 무료·무제한 공개 번역 엔드포인트를 클라이언트에서 직접 호출
     (API 키 불필요, 별도 과금 없음).
   · 한글이 포함된 줄은 번역하지 않는다(이미 한국어로 간주).
   · 여러 줄을 한 번의 요청으로 묶어 보내고 줄바꿈 기준으로 다시
     나눠 매칭한다 — 요청 수를 최소화해 더 빠르고 안정적으로 동작한다.
     매칭 개수가 어긋나면 해당 구간만 한 줄씩 재요청해 정확도를 보장한다.
──────────────────────────────────────────────────────────────── */
const LV_TRANS_CACHE = {}; // videoId -> [translatedText|null, ...] (LY.lines와 동일 인덱스)

function _lineNeedsTranslation(text) {
    if (!text) return false;
    if (/[\uAC00-\uD7A3]/.test(text)) return false; // 한글 포함 → 번역 불필요
    if (!/[A-Za-z\u00C0-\u024F\u3040-\u30FF\u4E00-\u9FFF\u0400-\u04FF]/.test(text)) return false; // 실제 문자 없음(기호 등)
    return true;
}

async function _translateBatch(text) {
...
async function _ensureLvTranslations(videoId, lines) {
    if (LV_TRANS_CACHE[videoId]) { _applyLvTranslationsAll(LV_TRANS_CACHE[videoId]); return LV_TRANS_CACHE[videoId]; }
    const idxToTranslate = [];
    lines.forEach((l, i) => { if (_lineNeedsTranslation(l.text)) idxToTranslate.push(i); });
    const result = new Array(lines.length).fill(null);
    if (!idxToTranslate.length) { LV_TRANS_CACHE[videoId] = result; return result; }

    const CHUNK = 20;
    for (let c = 0; c < idxToTranslate.length; c += CHUNK) {
        if (LY.videoId !== videoId) return result; // 곡이 바뀌면 중단
        const chunkIdx = idxToTranslate.slice(c, c + CHUNK);
        const joined = chunkIdx.map(i => lines[i].text).join('\n');
        let parts = [];
        try {
            const translated = await _translateBatch(joined);
            parts = translated.split('\n');
        } catch (e) { parts = []; }
        if (parts.length !== chunkIdx.length) {
            // 줄 수가 어긋나면 해당 구간만 한 줄씩 재요청 (정확도 우선)
            parts = [];
            for (const i of chunkIdx) {
                if (LY.videoId !== videoId) return result;
                try { parts.push(await _translateBatch(lines[i].text)); }
                catch (e) { parts.push(''); }
            }
        }
        chunkIdx.forEach((lineIdx, k) => { result[lineIdx] = (parts[k] || '').trim(); });
        if (LY.videoId === videoId) _applyLvTranslationsPartial(chunkIdx, result);
    }
    LV_TRANS_CACHE[videoId] = result;
    return result;
}

function _applyLvTranslationsPartial(idxArr, result) {
    if (!NP_LV.active) return;
    let changed = false;
    idxArr.forEach(i => {
        const el = LY.lines[i]?.el_lv_trans;
        const txt = result[i];
        if (!el || !txt) return;
        el.textContent = txt;
        el.classList.add('lv-ready');
        changed = true;
    });
    // 번역이 추가되며 줄 높이가 바뀌었으므로, 현재 줄이 계속 화면 중앙에
    // 오도록 스프링 목표 지점을 다시 계산한다(부드럽게 재보정).
    if (changed) requestAnimationFrame(() => _highlightLvLine(LY.curIdx, false));
}

function _applyLvTranslationsAll(result) {
    if (!NP_LV.active) return;
    let changed = false;
